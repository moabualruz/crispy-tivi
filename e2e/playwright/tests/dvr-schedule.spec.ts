...
const SS_DIR = path.join(REPORT_DIR, "screenshots", "dvr-schedule-crawl");
...
test.describe("DVR Schedule Flow", () => {
  test("DVR screen shows 4 tabs, storage bar, FAB, and schedule dialog", async ({
    page,
  }) => {
    page.on("console", (msg) => {
      const text = msg.text();
    ...
    if (onboarding) {
      log(
        "App shows onboarding (no sources configured) — " +
          "DVR screen is not reachable without sources",
      );
      // Verify the onboarding screen renders without crash.
      ...
      // Write crawl report and exit early — test passes.
      const content = [
        "# DVR Schedule Crawl Logs (skipped — no sources)",
      ...
      fs.writeFileSync(
        path.join(REPORT_DIR, "dvr-schedule-crawl-logs.txt"),
        content,
      );
    ...
    for (const tabLabel of DVR_TABS) {
      try {
        ...
        try {
          const semanticsTab = page.locator(`[aria-label*="${tabLabel}"]`);
          const count = await semanticsTab.count();
          if (count > 0) {
            tabsFound++;
            log(`DVR tab found via aria-label: "${tabLabel}"`);
          } else {
    ...
    for (const label of storageLabels) {
      ...
      try {
        ...
        try {
          const semanticsEl = page.locator(`[aria-label*="${label}"]`);
          const count = await semanticsEl.count();
          if (count > 0) {
            storageBarFound = true;
            log(`Storage indicator found via aria-label: "${label}"`);
          }
        } catch {
          // Try next label.
        }
    ...
    if (!storageBarFound) {
      ...
      try {
        ...
        if (count > 0) {
          storageBarFound = true;
          log(`Storage bar found via role=progressbar (count: ${count})`);
        }
      } catch {
        // Ignore.
      }
    ...
    for (const label of fabLabels) {
      ...
      try {
        ...
        try {
          const semanticsFab = page.locator(`[aria-label*="${label}"]`);
    ...
    if (!fabFound) {
      ...
      try {
        const fabCoord = page.locator('[role="button"][aria-label]');
    ...
    for (const label of scheduleLabels) {
      ...
      try {
        ...
        try {
          const semanticsBtn = page.locator(`[aria-label*="${label}"]`);
          const count = await semanticsBtn.count();
          if (count > 0) {
            scheduleOptionVisible = true;
            log(`Schedule option found via aria-label: "${label}"`);
    ...
    if (!scheduleOptionVisible) {
      ...
      await page.mouse.click(COORD.scheduleOption.x, COORD.scheduleOption.y);
      scheduleOptionVisible = true;
    ...
    try {
      ...
      try {
        ...
        if (count > 0) {
          dialogHasFields = true;
          log(`Schedule dialog fields found via aria-label (count: ${count})`);
        }
      } catch {
