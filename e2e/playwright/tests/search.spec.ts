...
const SS_DIR = path.join(REPORT_DIR, "screenshots", "search-crawl");
...
test.describe("Search Flow", () => {
  test("search field accepts input and shows results", async ({ page }) => {
    ...
    if (onboarding) {
      ...
      fs.writeFileSync(path.join(REPORT_DIR, "search-crawl-logs.txt"), content);
    ...
    try {
      ...
      if (isCompact) {
        ...
        await page.mouse.click(COORD.search.x, COORD.search.y);
      }
      searchNavDone = true;
    }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          await page.mouse.click(COORD.searchField.x, COORD.searchField.y);
