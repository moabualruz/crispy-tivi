...
test.describe('Responsive Layout', () => {
  ...
  test(
    'mobile viewport uses bottom navigation bar',
    async ({ page }, testInfo) => {
      ...
      if (navCount > 0) {
        ...
        if (firstNav && viewport) {
          ...
          expect(firstNav.y).toBeGreaterThanOrEqual(
            bottomThreshold,
          );
        }
      }
  ...
  test(
    'desktop viewport uses side navigation rail',
    async ({ page }, testInfo) => {
      const viewport = page.viewportSize();
      const width = viewport?.width ?? 0;
      ...
      if (navCount > 0) {
        ...
        if (firstNav && viewport) {
          ...
          expect(firstNav.y).toBeLessThan(
            viewport.height * 0.6,
          );
        }
      }
  ...
  test(
    'takes comparison screenshots at each viewport',
    async ({ page }, testInfo) => {
  ...
  test(
    'layout adapts correctly to viewport dimensions',
    async ({ page }, testInfo) => {
      const viewport = page.viewportSize();
      if (!viewport) {
        test.skip();
      ...
      expect(bodyBox!.width).toBeGreaterThanOrEqual(
        viewport.width - 1,
      );
