...
const SS_DIR = path.join(REPORT_DIR, "screenshots", "favorites-crawl");
...
test.describe("Favorites Flow", () => {
  test("Favorites screen shows 4 tabs with correct empty states", async ({
    page,
  }) => {
    page.on("console", (msg) => {
      const text = msg.text();
    ...
    try {
      ...
      await page.mouse.click(COORD.favorites.x, COORD.favorites.y);
      navDone = true;
    }
    ...
    for (const tabLabel of FAVORITES_TABS) {
      ...
      for (const phrase of emptyPhrases) {
        ...
        try {
          ...
          log(`Empty state found for "${tabLabel}" via text: "${phrase}"`);
        } catch {
          // Try next phrase.
        }
      }
  ...
  test("long-pressing a Live TV channel exposes Add to Favorites option", async ({
    page,
  }) => {
    page.on("console", (msg) => {
      const text = msg.text();
    ...
    if (onboarding) {
      ...
      fs.mkdirSync(REPORT_DIR, { recursive: true });
      fs.writeFileSync(
        path.join(REPORT_DIR, "favorites-contextmenu-crawl-logs.txt"),
        content,
      );
      const appErrors = filterAppErrors(errors);
      expect(appErrors).toHaveLength(0);
    ...
    try {
      ...
      // Coordinate fallback.
      await page.mouse.click(COORD.firstChannel.x, COORD.firstChannel.y, {
        button: "right",
      });
      contextMenuOpened = true;
      ...
      try {
        ...
        try {
          ...
          if (count > 0) {
            ...
            log(`"${label}" found and clicked via aria-label`);
          }
        } catch {
          // Try next label.
        }
