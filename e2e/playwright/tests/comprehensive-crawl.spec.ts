...
const SCREENSHOT_DIR = path.join(LOG_DIR, "screenshots", "crawl");
...
async function tryClickAria(
  ...
  try {
    const loc = page.locator(
      `[aria-label="${label}"], ` + `flt-semantics[aria-label="${label}"]`,
    );
...
test.describe("Comprehensive QA Crawl", () => {
  ...
  test("full app crawl with screenshots and logs", async ({ page }) => {
    setupListeners(page);
    ...
    const profileItems = await page
      .locator('[aria-label*="Profile"], ' + '[aria-label="Default"]')
      .count()
      .catch(() => 0);
    logMsg(`Profile items found: ${profileItems}`);

    // Select default profile
    const profileSelected = await selectDefaultProfile(page);
    ...
    for (const tab of NAV_TABS) {
      ...
      await page.waitForTimeout(2000);
      await screenshot(page, `tab-${tab.toLowerCase()}`);
    ...
    for (const settingItem of [
      ...
      const clicked = await tryClick(page, settingItem, 3000);
      if (clicked) {
        await screenshot(page, `settings-${settingItem.toLowerCase()}`);
        logMsg(`Settings section "${settingItem}": ` + `visible`);
      }
    }
    ...
    for (const addBtn of [
      ...
      if (vis) {
        ...
        await screenshot(
          page,
          `settings-${addBtn.toLowerCase().replace(/\s+/g, "-")}`,
        );
    ...
    (await tryClick(page, "Live TV")) || (await tryClickAria(page, "Live TV"));
    await page.waitForTimeout(2000);
    ...
    const channelCount = await channelItem.count().catch(() => 0);
    ...
    const viewport = page.viewportSize();
    logMsg(`Viewport: ${viewport?.width}x` + `${viewport?.height}`);
    ...
    (await tryClick(page, "Movies")) || (await tryClickAria(page, "Movies"));
    await page.waitForTimeout(2000);
    ...
    (await tryClick(page, "Guide")) || (await tryClickAria(page, "Guide"));
    await page.waitForTimeout(2000);
    ...
    fs.writeFileSync(path.join(LOG_DIR, "crawl-logs.txt"), logContent);

    logMsg(
      `Total errors: ${allErrors.length}, ` + `warnings: ${allWarnings.length}`,
    );
  });
});
