...
class EmbySeriesScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EmbySeriesScreen> createState() => _EmbySeriesScreenState();
}

class _EmbySeriesScreenState extends ConsumerState<EmbySeriesScreen>
    with SingleTickerProviderStateMixin {
  TabController? _tabController;
  int _selectedSeasonIndex = 0;

  /// Currently selected season ID — drives [embyEpisodesProvider].
    ...
    if (episode.type == MediaType.episode ||
      ...
      context.push(
        ...
        extra: {
          ...
          'heroTag': 'emby_series_${episode.id}',
        },
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    // EB-FE-11: watch seasons for this series.
    final seasonsAsync = ref.watch(embySeasonsProvider(widget.seriesId));
...
/// [EB-FE-11] Vertical list of episodes for the selected season.
class _EpisodesPane extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // EB-FE-11: watch episodes for the selected season.
    final episodesAsync = ref.watch(embyEpisodesProvider((seriesId, seasonId)));
...
/// [EB-FE-11] Single episode row with thumbnail, badge, title, runtime,
...
/// AppBar title widget for the series screen.
class _BreadcrumbTitle extends StatelessWidget {
  const _BreadcrumbTitle({required this.title});
