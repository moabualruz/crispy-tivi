...
public class AppShellViewModelTests
{
    ...
    [Fact]
    public void ErrorNotification_Defaults_AreHidden()
    {
        var sut = Build();
        sut.IsErrorNotificationVisible.Should().BeFalse();
        sut.ErrorNotificationMessage.Should().BeNull();
    }

    [AvaloniaFact]
    public void ShowsErrorNotification_WhenPlayerErrorMessageSet()
    {
        // Arrange — need a controllable state subject to emit errors
        var stateSubject = new TestSubject<PlayerState>();
        var playerService = Substitute.For<IPlayerService>();
        playerService.State.Returns(PlayerState.Empty);
        playerService.StateChanged.Returns(stateSubject);
        ...
        var player = new PlayerViewModel(playerService, timeshiftService, sleepTimer);
        var nav = Substitute.For<INavigationService>();
        nav.CanGoBack.Returns(false);
        nav.CurrentViewModel.Returns((ViewModelBase?)null);
        var sut = new AppShellViewModel(player, new MainViewModel(nav));

        // Act — emit error state
        stateSubject.OnNext(PlayerState.Empty with
        {
            ErrorMessage = "GStreamer not available. Install GStreamer 1.24+ runtime.",
        });

        // Assert
        sut.IsErrorNotificationVisible.Should().BeTrue(
            "error notification must be visible when player has an error");
        sut.ErrorNotificationMessage.Should().Be("GStreamer not available. Install GStreamer 1.24+ runtime.");
    }

    [Fact]
    public void DismissErrorNotification_HidesNotification()
    {
        var sut = Build();
        // Simulate showing a notification
        sut.IsErrorNotificationVisible = true;
        sut.ErrorNotificationMessage = "Test error";

        sut.DismissErrorNotificationCommand.Execute(null);
