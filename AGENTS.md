...
## CrispyTivi project details

### Intent
- Work directly in this repository without relying on any external `.ai` folder or submodule.
- Use only checked-in project files as context.

### Project boundaries
- Rust owns backend logic, sync, parsing, validation, FFI-facing storage behavior, and persistence-heavy flows.
- Flutter owns UI, navigation, interactions, platform presentation, and render-time state behavior.
...
## CrispyTivi design-system rules

- Flutter tokens in `app/flutter/lib/core/theme/` are authoritative; Penpot and exported token JSON mirror those names and values.
- Do not add visual tokens before checking existing `Crispy*` color, spacing, radius, typography, elevation, and animation tokens.
- Use checked-in design-system docs under `design/docs/` and workflow skills under `.codex/skills/*design-system/` before editing Penpot, Widgetbook, or shared UI primitives.
- Widgetbook coverage lives in `design/docs/widgetbook-coverage.md` and is generated by `scripts/design/generate_widgetbook_coverage.py`.
- Add direct `@widgetbook.UseCase` entries only for widgets with stable fixtures; mark provider-heavy widgets as `deferred-provider-fixture` with concrete family/harness reasons.
- Penpot work must target the active `CrispyTivi Design System` page, use editable shapes as primary artifacts, attach `crispy-tivi` shared plugin data, and verify token/board/asset read-back through the local REPL when available.
