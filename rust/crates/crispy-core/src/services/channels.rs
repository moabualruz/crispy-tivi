...
impl CrispyService {
    ...
    pub fn save_channels(&self, channels: &[Channel]) -> Result<usize, DbError> {
        ...
        for ch in channels {
            let sid = ch.source_id.clone().unwrap_or_default();
            if seen.insert(sid.clone()) {
                self.emit(DataChangeEvent::ChannelsUpdated { source_id: sid });
            }
        }
    ...
    pub fn delete_removed_channels(
        ...
        let conn = self.db.get()?;
        let tx = conn.unchecked_transaction()?;
        let deleted = super::delete_removed_by_source(&tx, "db_channels", source_id, keep_ids)?;
        tx.commit()?;
        self.emit(DataChangeEvent::ChannelsUpdated {
