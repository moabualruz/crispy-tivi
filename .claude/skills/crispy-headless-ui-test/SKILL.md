---
name: crispy-headless-ui-test
description: Write correct Avalonia headless UI tests for CrispyTivi Views and Controls with code-behind. Use this whenever writing, fixing, or reviewing UI tests — especially when tests fail with FindControl errors, System.Reactive missing, SeekBarHost null, or pointer events not propagating. Also use when creating tests for any View or Control that has code-behind logic.
---
...
### AXAML-Generated Fields and InitializeComponent
...
Never use `FindControl<T>(name)` in production code-behind. Use AXAML-generated fields directly.

If a field is null in tests, check in order:
1. `InitializeComponent()` is called in the constructor
2. The view is attached to a window (use `CreateWindow`, not `new FooView()`)
3. DataContext is set before state is pushed

### OnAttachedToVisualTree Timing

`x:Name` fields are only guaranteed to be non-null **after** the visual tree is attached. Code in `OnAttachedToVisualTree` fires after `InitializeComponent()` completes and the control is added to the window.

If your code-behind subscribes to events or accesses named controls in `OnAttachedToVisualTree`, always use `CreateWindow` to trigger that lifecycle — `new FooView()` alone will NOT call `OnAttachedToVisualTree`.

```csharp
// This triggers the full lifecycle including OnAttachedToVisualTree
var window = HeadlessTestHelpers.CreateWindow<FooView>(vm);
// Named fields and subscriptions are now live
```
...
Use `KeyPressQwerty` to simulate key presses on the window:

```csharp
window.KeyPressQwerty(Key.Space);
```

To assert focus, use `FocusManager`:

```csharp
var focused = FocusManager.GetFocusManager(window)?.GetFocusedElement();
focused.Should().Be(expectedControl);
```
...
4. Check `OnAttachedToVisualTree` has fired (only after `CreateWindow`)
5. Fix the setup — do NOT weaken or skip the assertion
