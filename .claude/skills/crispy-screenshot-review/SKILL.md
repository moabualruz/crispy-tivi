...
description: AI reviews CrispyTivi Flutter golden test failures against journey specs and design docs. Classifies each failure as regression, improvement, neutral, or spec violation. Use when asked to "review screenshots", "check visual regressions", "review test run", "what broke visually", "analyze screenshot failures". Triggers on: review screenshots, screenshot failures, visual regression review.
---

# AI Screenshot Review — CrispyTivi Flutter

Review golden test failures by comparing actual renders against journey specifications and design documents.

## Steps

1. **Load test run data:**
   - Read `test/output/{pipeline}/runs-index.json` to find latest run
   - Read `{latest_path}/manifest.json`

2. **Load design context:**
   - Read `F:/work/crispi-tv-flutter/.ai/crispy_tivi_design_spec.md` for visual design rules
   - Read `F:/work/crispi-tv-flutter/.impeccable.md` for design context
   - Read `F:/work/crispi-tv-flutter/.ai/planning/USER-JOURNEYS.md` for journey definitions
...
4. **For each `new` screenshot (no baseline):**
...
5. **Output structured review:**
   ...
   | j05/007_focus_ring | IMPROVEMENT | approve | Focus ring now matches design spec |
...
6. **Cross-reference git changes:**
   - For regressions, check `git log --oneline -10` for recent changes to `lib/` and `test/`
...
- Flutter golden diffs can be pixel-level on font rendering — check if the diff is meaningful layout change vs font hinting artifact
