---
name: crispy-qa-e2e
description: Run full E2E tests with real IPTV sources using Flutter integration tests, then analyze screenshots and logs against CrispyTivi journey specs and design docs. Use when asked to "run e2e tests", "test with real data", "full QA", "e2e validation", "test real sources", "end-to-end test". Triggers on: e2e tests, real source test, full QA pipeline.
---

# E2E Visual QA Pipeline — CrispyTivi Flutter

Run Pipeline 3 (E2E) — fresh DB, real source sync, full user journey simulation via Flutter integration tests.

## Prerequisites
- `test/fixtures/test-settings.local.json` must exist with real IPTV source credentials
- Network access to IPTV servers required
- A connected device or emulator must be running (or use `flutter test` for widget/golden tests)

## Steps

1. **Verify prerequisites:**
   ```bash
   ls test/fixtures/test-settings.local.json
   flutter devices
   ```
   If fixture missing, ask the user for source credentials before proceeding.

2. **Run E2E pipeline:**
   ```bash
   # Widget/golden tests (no device needed):
   CRISPY_PIPELINE=e2e flutter test test/golden/ --update-goldens

   # Integration tests (requires device/emulator):
   CRISPY_PIPELINE=e2e flutter test integration_test/ --dart-define=PIPELINE=e2e
   ```
   E2E with real sync takes 5–15 minutes.

3. **Find latest results:**
   - Read `test/output/e2e/runs-index.json`
...
4. **Analyze screenshots:**
   ...
   - Compare against:
     - `F:/work/crispi-tv-flutter/.ai/planning/USER-JOURNEYS.md` — journey step expectations
     - `F:/work/crispi-tv-flutter/.ai/crispy_tivi_design_spec.md` — visual design rules
     - `F:/work/crispi-tv-flutter/.impeccable.md` — design principles
...
6. **Cross-reference visual + log issues:**
   ...
   - Find the log event that explains WHY (network error? missing data? wrong provider state?)
...
- Network timeout: E2E sync timeout is 120s. If exceeded, check credentials in `test/fixtures/test-settings.local.json`
